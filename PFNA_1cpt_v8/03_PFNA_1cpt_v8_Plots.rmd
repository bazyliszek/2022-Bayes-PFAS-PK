---
title: "PFNA 1 compartment Plots (v8)"
...
load("PFNA_1cpt_v8_multicheck.Rdata")
```
...
p<- ggplot(dwc.train.df)+
  ...
  geom_hline(aes(yintercept = 500/10,
...
ggsave(here ("output-plots",paste0( sa, "PFNA_DWC_below_MRL.pdf")) ,p,dpi=600)
ggsave(here ("output-plots",paste0( sa, "PFNA_DWC_below_MRL.png")) ,p,dpi=600)
save(p,dwc.train.df,file=here("output-plots",paste0( sa, "PFNA_DWC_below_MRL.RData")))
...
write.csv(as.matrix(dwc.train.q),
          file=here("output-plots",paste0( sa, "PFNA_DWC_below_MRL.csv")))
...
p<-ggplot()+
  stat_density(aes(halflife_GM, color = "Prior",linetype="Prior"),data=PFNA_priors,geom="line",size=1)+
  stat_density(aes(PFNA_halflife_GM,stat(density),color="Posterior",linetype="Posterior"),geom="line",size=1.5)+
  xlim(0,15)+
...
save(p,PFNA_priors,PFNA_halflife_GM,PFNA_hl_median_gm,PFNA_hl_median_gm_95ci,file=here("output-plots",paste0( sa, "PFNA_hl_gm.RData")))
```



```{r PFNA-half-life-gsd, message=FALSE, warning=FALSE,fig.height=2.5,fig.width=4,dpi=600}
...
PFNA_hl_gsd_med_95ci <- paste(signif(quantile(PFNA_halflife_GSD,
                                            prob=c(0.025,0.975)),3),collapse="-")
p<-ggplot()+
  stat_density(aes(halflife_GSD, color = "Prior",linetype="Prior"),data=PFNA_priors,geom="line",size=1)+
  stat_density(aes(PFNA_halflife_GSD,stat(density), color = "Posterior",linetype="Posterior"),geom="line",size=1.5)+
  xlim(1,3)+
...
save(p,PFNA_priors,PFNA_halflife_GSD,PFNA_hl_gsd_med,PFNA_hl_gsd_med_95ci,file=here("output-plots",paste0( sa, "PFNA_hl_gsd.RData")))
```
...
```{r PFNA-vd-gm, message=FALSE, warning=FALSE,fig.height=2.5,fig.width=4,dpi=600}
...
p<-ggplot()+
  stat_density(aes(Vd_GM, color = "Prior",linetype="Prior"),data=PFNA_priors,geom="line",size=1)+
  stat_density(aes(PFNA_Vd_GM,stat(density), color = "Posterior",linetype="Posterior"),geom="line",size=1.5)+
...
save(p,PFNA_priors,PFNA_Vd_GM,PFNA_vd_gm_med,PFNA_vd_gm_med_95ci,file=here("output-plots",paste0( sa, "PFNA_vd_gm.RData")))
```


```{r PFNA-vd-sd, message=FALSE, warning=FALSE,fig.height=2.5,fig.width=4,dpi=600}
...
p<-ggplot()+
  stat_density(aes(Vd_GSD, color = "Prior",linetype="Prior"),data=PFNA_priors,geom="line",size=1)+
  stat_density(aes(PFNA_Vd_GSD,stat(density), color = "Posterior",linetype="Posterior"),geom="line",size=1.5)+
  xlim(1,3)+
...
save(p,PFNA_priors,PFNA_Vd_GSD,PFNA_vd_gsd_med,PFNA_vd_gsd_med_95ci,file=here("output-plots",paste0( sa, "PFNA_vd_gsd.RData")))
```
### Clearance

Cl is k * Vd

```{r PFNA-cl-gm, message=FALSE, warning=FALSE,fig.height=2.5,fig.width=4,dpi=600}
...
p<-ggplot()+
  stat_density(aes(CL_GM, color = "Prior",linetype="Prior"),data=PFNA_priors,geom="line",size=1)+
  stat_density(aes(PFNA_CL_GM,stat(density), color = "Posterior",linetype="Posterior"),geom="line",size=1.5)+
  xlim(0,0.25)+labs(title = "E: PFNA CL Population GM ",subtitle=paste("Posterior Median (95% CI): ",
                                                                    PFNA_cl_gm_med," (",
                                                                    PFNA_cl_gm_med_95ci,
                                                                    ")",sep=""))+
...
save(p,PFNA_priors,PFNA_CL_GM,PFNA_cl_gm_med,PFNA_cl_gm_med_95ci,file=here("output-plots",paste0( sa, "PFNA_CL_gm.RData")))
```

```{r PFNA-CL-sd, message=FALSE, warning=FALSE,fig.height=2.5,fig.width=4,dpi=600}
...
p<-ggplot()+
  stat_density(aes(CL_GSD, color = "Prior",linetype="Prior"),data=PFNA_priors,geom="line",size=1)+
  stat_density(aes(PFNA_CL_GSD,stat(density), color = "Posterior",linetype="Posterior"),geom="line",size=1.5)+
  xlim(1,3)+
  labs(title = bquote("F: PFNA"~CL~"Population GSD "),
       subtitle=paste("Posterior Median (95% CI): ",
                      PFNA_CL_gsd_med," (",
...
save(p,PFNA_priors,PFNA_CL_GSD,PFNA_CL_gsd_med,PFNA_CL_gsd_med_95ci,file=here("output-plots",paste0( sa, "PFNA_CL_gsd.RData")))
```
...
| -------------------------------|:-------------:|:------------:|:-------------:|:-------------:| 
| Half-life (years)  | `r paste(PFNA_hlgm_pr_med) ` |   `r paste(PFNA_hl_median_gm) `    | `r  paste(PFNA_hlgsd_pr_med) `           | `r paste(PFNA_hl_gsd_med)`  | 
| HL  [95% CI]  |`r paste0("[",PFNA_hlgm_pr_med_95ci,"]")` | `r paste0( "[",PFNA_hl_median_gm_95ci,"]")`| `r paste0("[",PFNA_hlgsd_pr_med_95ci,"]")`| `r paste0( "[",PFNA_hl_gsd_med_95ci, "]")`|
| Volume of distribution    | `r paste(PFNA_vd_gm_pr_med) ` |    `r paste(  PFNA_vd_gm_med) `|   `r  paste(PFNA_vd_gsd_pr_med)`  |`r  paste(PFNA_vd_gsd_med)` |  
| $V_D$ [95% CI]    |`r paste0( "[",PFNA_vd_gm_pr_med_95ci,"]")`| `r paste0( "[",PFNA_vd_gm_med_95ci,"]")`| `r paste0("[",PFNA_vd_gsd_pr_med_95ci, "]")`| `r paste0(  "[",PFNA_vd_gsd_med_95ci, "]")`| 
| Clearance    | `r paste(PFNA_cl_gm_pr_med) ` |    `r paste(  PFNA_cl_gm_med) `|   `r  paste()`  |`r  paste()` |  
| $CL$ [95% CI]    |`r paste0( "[",PFNA_cl_gm_pr_med_95ci,"]")`| `r paste0( "[",PFNA_cl_gm_med_95ci,"]")`| `r paste0("[", "]")`| `r paste0(  "[", "]")`| 
